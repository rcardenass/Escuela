...
$maxRows_rsMensajes = 5;
$pageNum_rsMensajes = 0;
if (isset($_GET['pageNum_rsMensajes'])) {
  $pageNum_rsMensajes = $_GET['pageNum_rsMensajes'];
}
$startRow_rsMensajes = $pageNum_rsMensajes * $maxRows_rsMensajes;

mysql_select_db($database_cn, $cn);
$query_rsMensajes = "SELECT a.CodMensaje, a.De, a.FechaEnvio AS Fecha,  a.Asunto, b.Mensaje FROM mensaje a INNER JOIN detallemensaje b ON b.CodMensaje=a.CodMensaje INNER JOIN usuario c ON c.Email=a.Para WHERE a.CodCurGra='".$row_rsCursos['CodCurGra']."' AND a.Para='$Usuario' ";
$query_limit_rsMensajes = sprintf("%s LIMIT %d, %d", $query_rsMensajes, $startRow_rsMensajes, $maxRows_rsMensajes);
$rsMensajes = mysql_query($query_limit_rsMensajes, $cn) or die(mysql_error());
$row_rsMensajes = mysql_fetch_assoc($rsMensajes);

if (isset($_GET['totalRows_rsMensajes'])) {
  $totalRows_rsMensajes = $_GET['totalRows_rsMensajes'];
} else {
  $all_rsMensajes = mysql_query($query_rsMensajes);
  $totalRows_rsMensajes = mysql_num_rows($all_rsMensajes);
}
$totalPages_rsMensajes = ceil($totalRows_rsMensajes/$maxRows_rsMensajes)-1;
              ...
              <tr>
                <td>
                  <div style="width:500px; height:70px">
                      <div style="width:80px; height:70px; float:left; background-color:#99FFCC">
					  	<?php echo $row_rsMensajes['De']; ?><br/><?php echo $row_rsMensajes['Fecha']; ?>                      
					  </div>
				      <div style="width:415px; height:70px; float:right; padding-right::80px; text-align:left; background-color:#FFFFCC">
				        <?php echo $row_rsMensajes['Asunto']; ?><br/>
				        <?php echo $row_rsMensajes['Mensaje']; ?>			          </div>
				        </div></td>
              </tr>
