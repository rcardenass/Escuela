...
/*$query_rsCabecera = "SELECT a.CodAlumno, Concat(a.ApellidoPaterno,' ',a.ApellidoMaterno,' ',a.Nombres) AS Alumno, a.Direccion,  concat(b.NombreDepartamento,'-', c.NombreProvincia,'-',d.NombreDistrito) as Ubigeo, DATE_FORMAT(a.FechaNacimiento,'%d/%m/%Y') AS FecNac, a.Telefono, a.EmailPersonal AS Email FROM alumno a INNER JOIN departamento b ON b.CodDepartamento=a.CodDepartamento INNER JOIN provincia c ON c.CodProvincia=a.CodProvincia INNER JOIN distrito d ON d.CodDistrito=a.CodDistrito WHERE a.CodAlumno='".$_SESSION['CajaCodAlumno']."' ";*/
$query_rsCabecera = "SELECT a.CodAlumno, Concat(a.ApellidoPaterno,' ',a.ApellidoMaterno,' ',a.Nombres) AS Alumno, a.Direccion,  concat(
(SELECT x.Nombre FROM ubigeo x WHERE x.CodDepartamento<>'00' AND x.CodProvincia='00' AND x.CodDistrito='00' AND x.CodDepartamento=LEFT(a.Ubigeo,2)),' - ', (SELECT x.Nombre FROM ubigeo x WHERE x.CodDepartamento<>'00' AND x.CodProvincia<>'00' AND x.CodDistrito='00' AND x.CodDepartamento=LEFT(a.Ubigeo,2) AND x.CodProvincia=RIGHT(LEFT(a.Ubigeo,4),2)),' - ', (SELECT x.Nombre FROM ubigeo x WHERE x.CodDepartamento<>'00' AND x.CodProvincia<>'00' AND x.CodDistrito<>'00' AND x.CodDepartamento=LEFT(a.Ubigeo,2) AND x.CodProvincia=RIGHT(LEFT(a.Ubigeo,4),2) AND x.CodDistrito=RIGHT(a.Ubigeo,2)) ) AS Ubigeo, DATE_FORMAT(a.FechaNacimiento,'%d/%m/%Y') AS FecNac, a.Telefono, a.EmailPersonal AS Email FROM alumno a WHERE a.CodAlumno='".$_SESSION['CajaCodAlumno']."' ";
...
mysql_select_db($database_cn, $cn);
$query_rsDeudaPension = "SELECT SUM(a.Monto-a.Pagado) AS Pagar FROM programacionalumno a  INNER JOIN anio b ON b.CodAnio=a.CodAnio  INNER JOIN grado c ON c.CodGrado=a.CodGrado  WHERE a.CodAlumno='".$_SESSION['CajaCodAlumno']."' AND a.Estado=1  AND a.Monto+a.Mora>a.Pagado ";
...
mysql_select_db($database_cn, $cn);
$query_rsMoraPension = "SELECT SUM(a.Mora) AS Pagar FROM programacionalumno a  INNER JOIN anio b ON b.CodAnio=a.CodAnio  INNER JOIN grado c ON c.CodGrado=a.CodGrado  WHERE a.CodAlumno='".$_SESSION['CajaCodAlumno']."' AND a.Estado=1  AND a.Monto+a.Mora>a.Pagado AND DATE_FORMAT(FechaTermino,'%d/%m/%Y')<DATE_FORMAT(now(),'%d/%m/%Y')";
