...
mysql_select_db($database_cn, $cn);
$query_rsEditar = "SELECT CodUsuario, Codigo AS CodPadreFamilia, Estado FROM usuario WHERE CodUsuario='".$_GET['Codigo']."' GROUP by Codigo ";
...
<table width="90%" border="0" align="center"><tr><td>
	<table width="100%" border="0" cellpadding="0">
		...
		<h1>
		<div style="width:100%">
			<div style="width:400px; height:auto; float:left; padding-top:20px">Editar Cuenta Intranet</div>
			<div style="width:100px; height:auto; float:right; text-align:right">
				<a href="cuepadre.php"><img src="../imagenes/icono/revert.png" border="0" title="Alumno con cuenta"/></a>
			</div>
		</div>
	    </h1><div style="height:30px"></div><hr /><br />
        <form action="cueupdatepadre.php" method="post" name="form1" id="form1" onsubmit="MM_validateForm('cboUsuario','','RisNum','txtPassword','','R','txtRePassword','','R');return document.MM_returnValue" autocomplete="Off">
...
?>
            ...
            <tr>
                ...
                <td><select name="cboUsuario" id="cboUsuario" style="width:250px" disabled="disabled">
                  <option value="" <?php if (!(strcmp("", $row_rsEditar['CodPadreFamilia']))) {echo "selected=\"selected\"";} ?>>Seleccione</option>
                  <?php
do {  
?>
                  <option value="<?php echo $row_rsPadre['CodPadreFamilia']?>"<?php if (!(strcmp($row_rsPadre['CodPadreFamilia'], $row_rsEditar['CodPadreFamilia']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsPadre['Padre']?></option>
                  <?php
...
?>
            ...
            <tr>
              <td><input name="txtCodPadre" type="hidden" id="txtCodPadre" value="<?php echo $row_rsEditar['CodPadreFamilia']; ?>" />
                <input name="txtCodUsuario" type="hidden" id="txtCodUsuario" value="<?php echo $row_rsEditar['CodUsuario']; ?>" /></td>
              <td>
			  	<div style="height:10px"></div>
