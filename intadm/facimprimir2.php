...
do{
	mysql_select_db($database_cn, $cn);
	$query_rsDetalle = "SELECT CASE b.Tipo 	WHEN 'Pension' THEN (SELECT concat('Pension - ',x.NroPension) FROM programacionalumno x WHERE x.CodProgramacionAlumno=b.Codigo) 	WHEN 'concepto' THEN (SELECT y.NombreProducto FROM productos y WHERE y.CodProducto=b.Codigo) WHEN 'Credito' THEN (SELECT x.NombreProducto 
						FROM productos x 
						INNER JOIN detallecuentacorriente y ON y.CodProducto=x.CodProducto 
						INNER JOIN cuentacorriente z ON z.CodCuentaCorriente=y.CodCuentaCorriente
						WHERE z.CodAlumno=a.CodAlumno
						AND y.CodDetalleCuentaCorriente=b.Codigo
						AND y.MontoPagar>MontoPagado) END AS Concepto, b.Cantidad, b.PrecioUnit, b.Cantidad*b.PrecioUnit AS Monto, b.Dscto, b.Mora, b.SubTotal FROM comprobante a INNER JOIN detallecomprobante b ON b.CodComprobante=a.CodComprobante WHERE a.CodAlumno='".$_SESSION['CajaCodAlumno']."' AND a.CodComprobante=".$row_Comprobante['CodComprobante']." ";
	...
	mysql_select_db($database_cn, $cn);
	$query_rsTotal = "SELECT SUM(b.SubTotal) as Total FROM comprobante a INNER JOIN detallecomprobante b ON b.CodComprobante=a.CodComprobante WHERE a.CodAlumno='".$_SESSION['CajaCodAlumno']."' AND a.CodComprobante=".$row_Comprobante['CodComprobante']." ";
  ...
  <tr>
    <td valign="top"><table width="440" border="0" cellspacing="1" cellpadding="0">
      <tr>
        ...
        <td width="284"><?php echo $row_rsCabecera['CodAlumno']; ?></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td><?php echo $row_rsCabecera['Datos']; ?></td>
      </tr>
      <tr>
        <td>Fecha - Hora</td>
