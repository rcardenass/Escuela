...
if($totalRows_rsRecibos>0){	//PREGUNTA SI EXISTE RECIBOS PARA GENERAR
	do{
		...
		//REGISTRA LA CABECERA DE CLIENTE PARA (CAJA RAPIDA)
		if($objDatos->InsertCliente($_POST['txtNombres'],$_POST['txtDireccion'],$_POST['txtTelefono'],$_SESSION['MM_Username'])==1){
			//OBTENER CODIGO MAXIMO DE CLIENTE (PARA CAJA RAPIDA)
			$rsCliente=$objDatos->ObtenerClienteSelId();
			$row_Cliente=$objDatos->PoblarClienteSelId($rsCliente);
		}
		
		//REGISTRA LA CABECERA DE COMPROBANTE
		if($objDatos->InsertComprobante($row_Cliente['CodCliente'],$row_Recibos['CodTipoComprobante'],$_SESSION['MM_Username'],
		$row_Caja['CodCaja'],$OperacionId,1)==1){
			
			//OBTIENE EL CODIGO DE LA CABECERA DEL COMPROBANTE
			$rsCodigo=$objDatos->ObtenerCodigoComprobanteSelId($row_Cliente['CodCliente'],$_SESSION['MM_Username']);
			$row_Codigo=$objDatos->PoblarCodigoComprobanteSelId($rsCodigo);
			
			//REGISTRA EL DETALLE DEL COMPROBANTE
			//if($objDatos->InsertDetalleComprobante($row_Codigo['CodComprobante'],$_SESSION['MM_Username'],$row_Cliente['CodCliente'],
			if($objDatos->InsertDetalleComprobante($row_Codigo['CodComprobante'],$_SESSION['MM_Username'],0,
			$row_Recibos['CodTipoComprobante'])==1){
				
				//LIMPIAR EL TEMPORAL DEL ALUMNO CON ESTE TIPO DE COMPROBANTE
				$objDatos->EliminarTemporal($_SESSION['MM_Username'],0,$row_Recibos['CodTipoComprobante']);
			}
		}else{
...
?>
<a href="faccomprobante3.php?xyz=<?=$OperacionId;?>&&pqr=<?=$row_Cliente['CodCliente'];?>" target="_parent">Comprobante Nuevo</a>