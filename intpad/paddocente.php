...
$rsDocente=$objDatos->ObtenerProfesorDeAlumnoSelAll($row_Matricula['CodAnio'], $row_Matricula['CodGrado'], $row_Matricula['CodSeccion']);
$row_Docente=$objDatos->PoblarProfesorDeAlumnoSelAll($rsDocente);
?>
...
<div style = "display: table; margin-left: auto; margin-right: auto; width: 90%" class="Menu">
    ...
    <div style = "float: right; height: 100%; width: 80%; overflow:hidden" class="Contenedor">
		...
		<h1>Profesores</h1><hr /><br />
		<?
                do{
                    echo "<span class='label'>".$row_Docente['Profesor']."</span>";
                    echo "<div style='height:5px'></div>";
                    $rsCurso=$objDatos->ObtenerCursoDocentePorAlumnoSelAll($row_Docente['CodPersonal'], $row_Matricula['CodAnio'], $row_Matricula['CodGrado'], $row_Matricula['CodSeccion']);
                    $row_Curso=$objDatos->PoblarCursoDocentePorAlumnoSelAll($rsCurso);
                    do{
                        ...
                        echo "</table>";
                        echo "</div>";
                    }while($row_Curso=$objDatos->PoblarCursoDocentePorAlumnoSelAll($rsCurso));

                }while($row_Docente=$objDatos->PoblarProfesorDeAlumnoSelAll($rsDocente));
                ?>
		<!-- InstanceEndEditable -->		
		</div>
		<div style="height:10px"></div>
