...
$row_Bimestre=$objDatos->PoblarBimestreSelAll($rsBimestre);

$rsArea=$objDatos->ObtenerAreaPorAlumnoSelAll($_SESSION['Hijo'], $row_Matricula['CodAnio'] ,$row_Matricula['CodGrado'], $row_Matricula['CodSeccion']);
$row_Area=$objDatos->PoblarAreaPorAlumnoSelAll($rsArea);
?>
...
$(document).ready(function() {
    ...
    $('#form, #fat, #form2').submit(function() {
        $.ajax({
            type: 'POST',
...
<div style = "display: table; margin-left: auto; margin-right: auto; width: 90%" class="Menu">
    ...
    <div style = "float: right; height: 100%; width: 80%; overflow:hidden" class="Contenedor">
		...
		<h1>Calificaci&oacute;n de mi Hijo</h1><hr /><br />
		<form id="form2" name="form2" method="post" autocomplete="Off" action="php/padcalificacion.php">
		  <table width="450" border="0" cellspacing="2" cellpadding="0">
            <tr>
              <td width="160"><span class="label">Bimestre</span></td>
            ...
            <tr>
              <td width="160"><label>
                <select name="cboBimestre" id="cboBimestre" style="width:150px">
                    ...
                    <? do{ ?>
                    <option value="<? echo $row_Bimestre['CodBimestre']; ?>"><? echo $row_Bimestre['NombreBimestre']; ?></option>
                    <? }while($row_Bimestre=$objDatos->PoblarBimestreSelAll($rsBimestre)); ?>
                </select>
              </label></td>
              <td><label>
                ...
                <option value="0">Seleccione</option>
                <? do{ ?>
                <option value="<? echo $row_Area['CodArea']; ?>"><? echo $row_Area['NombreArea']; ?></option>
                <? }while($row_Area=$objDatos->PoblarAreaPorAlumnoSelAll($rsArea)); ?>
                </select>
              </label></td>
